...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
     ...
     * @return список транзакций
     */
    List<Transaction> findByWalletAndCreatedAtBetween(Wallet wallet, LocalDateTime start, LocalDateTime end);
     ...
     * @return сумма
     */
    @Query("SELECT COALESCE(SUM(t.amount), 0) FROM Transaction t WHERE t.wallet.id = :walletId AND t.type = :type AND t.createdAt BETWEEN :start AND :end")
    BigDecimal sumByWalletIdAndTypeAndDateRange(@Param("walletId") Long walletId, @Param("type") TransactionType type,
    ...
    BigDecimal sumByWalletIdAndCategoryId(@Param("walletId") Long walletId, @Param("categoryId") Long categoryId);
     ...
     * @return список транзакций
     */
    @Query("SELECT t FROM Transaction t WHERE t.wallet.id = :walletId AND t.category.id IN :categoryIds ORDER BY t.createdAt DESC")
    List<Transaction> findByWalletIdAndCategoryIds(@Param("walletId") Long walletId, @Param("categoryIds") List<Long> categoryIds);
}
